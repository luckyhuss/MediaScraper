...
#
###
##### ABO 26/02/2016 : Evolutions for MediaScraper ##### 
...
class Cli:
    ...
    def processCommand(self):
        ...
        if command == "status":
        ...
        elif command == "queue_detail":
            if len(args) > 0:
                # search by package name
                get_packages(self.client.getQueueData(), args[0])
            else:
                get_packages(self.client.getQueueData(), "")

        elif command == "collector":
            print_packages(self.client.getCollectorData())

        elif command == "collector_detail":
            if len(args) > 0:
                # search by package name
                get_packages(self.client.getCollectorData(), args[0])
            else:
                get_packages(self.client.getCollectorData(), "")
            ...
            print self.client.addPackage(args[0], args[1:], Destination.Queue)
            ...
            print self.client.addPackage(args[0], args[1:], Destination.Collector)
...
def get_packages(data, packageName):
    """ABO 26/02/2016 : Evolution to get all packages"""
    for pack in data:
        for download in pack.links:
            if packageName == "":
                print "pid:{0}|fid:{1}|pname:{2}|fname:{3}|furl:{4}|status:{5}|size:{6}".format(download.fid, pack.pid, pack.name, download.name, download.url, download.statusmsg, download.format_size)
            elif pack.name == packageName:
                print "pid:{0}|fid:{1}|pname:{2}|fname:{3}|furl:{4}|status:{5}|size:{6}".format(download.fid, pack.pid, pack.name, download.name, download.url, download.statusmsg, download.format_size)
...
def print_commands():
    commands = [("status", _("Prints server status")),
        ...
        ("queue_detail", _("Prints downloads (details) in queue")),
        ("collector", _("Prints downloads in collector")),
        ("collector_detail", _("Prints downloads (details) in collector")),
